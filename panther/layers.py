...
from panther.networks import DirNIWNet
...
class PANTHERBase(nn.Module):
    ...
    def __init__(
        ...
        if (
            out == "allcat"
        ):  # Concatenates pi, mu, cov - This is the default mode for PANTHER used in the paper
            self.outdim = p + 2 * p * d

        elif (
            out == "weight_all_cat"
        ):  # Weights mu and cov by weights and that concatenate
            self.outdim = 2 * p * d
    ...
    def forward(self, S, mask=None):
        ...
        pis, mus, Sigmas, qqs = [], [], [], []
        pi, mu, Sigma, qq = self.priors.map_em(
            S, mask=mask, num_iters=self.L, tau=self.tau, prior=self.priors()
        )
        ...
        mus = torch.stack(
            mus, dim=3
        )  # mus: (n_batch x n_proto x instance_dim x n_head)
        Sigmas = torch.stack(
            Sigmas, dim=3
        )  # Sigmas: (n_batch x n_proto x instance_dim x n_head)
        qqs = torch.stack(qqs, dim=3)

        if self.out == "allcat":
            out = torch.cat(
                [pis.reshape(B, -1), mus.reshape(B, -1), Sigmas.reshape(B, -1)], dim=1
            )
            ...
            for h in range(self.H):
                ...
                mu_weighted = torch.bmm(pi, mu).squeeze(
                    dim=1
                )  # (n_batch, instance_dim)
                Sigma_weighted = torch.bmm(pi, Sigma).squeeze(
                    dim=1
                )  # (n_batch, instance_dim)

...
class SSLMetaArch(nn.Module):
    def __init__(self, cfg):
        ...
        self.ctxor_out_dim = cfg.ctxor.head_n_prototypes
        ...
        self.do_ctxor = cfg.ctxor.loss_weight > 0
        ...
        logger.info("OPTIONS -- CONTEXT ORACLE")
        if self.do_ctxor:
            #logger.info(f"OPTIONS -- DINO -- loss_weight: {cfg.dino.loss_weight}")
            #logger.info(f"OPTIONS -- DINO -- head_n_prototypes: {cfg.dino.head_n_prototypes}")
            #logger.info(f"OPTIONS -- DINO -- head_bottleneck_dim: {cfg.dino.head_bottleneck_dim}")
            #logger.info(f"OPTIONS -- DINO -- head_hidden_dim: {cfg.dino.head_hidden_dim}")
            self.ctxor_loss_weight = cfg.ctxor.loss_weight
            student_model_dict["ctxor_head"] = partial(
                DINOHead,
                in_dim=embed_dim,
                out_dim=cfg.ctxor.head_n_prototypes,
                hidden_dim=cfg.ctxor.head_hidden_dim,
                bottleneck_dim=cfg.ctxor.head_bottleneck_dim,
                nlayers=cfg.ctxor.head_nlayers,
            )()
            self.ctxor_loss = DINOLoss(self.ctxor_out_dim)
        else:
            logger.info("OPTIONS -- COTOR -- not using COTOR")
        logger.info(f"Context oracle OK.")
        ...
        self.context_oracle = torchvision.models.resnet34()
        self.context_oracle.fc = torch.nn.Identity()
        co_ckpt = torch.load("/nfs/turbo/umms-tocho-snr/exp/akhilk/fast_glioma/patch_encoder/patch_encoder/a955fdfd_Nov17-09-42-47_sd1000_hidisc_skip_3ch_tune0/models/ckpt-epoch799-step18400-loss0.00.ckpt", map_location="cpu")
        self.context_oracle.load_state_dict({i.removeprefix("model.bb."): co_ckpt["state_dict"][i] for i in co_ckpt["state_dict"] if "bb" in i})

        for p in self.context_oracle.parameters():
            p.requires_grad = False
    ...
    def forward_(self, images, teacher_temp, return_student_emb=False):
        ...
        if self.do_ctxor:
            pctxor_local = self.student.ctxor_head(student_local_cls_tokens).chunk(n_local_crops)
            pctxor_global = self.student.ctxor_head(student_global_cls_tokens).chunk(2)
        ...
        if n_local_crops > 0:
            ...
            #loss_accumulator += 0 * dino_local_crops_loss
        ...
        if do_dino:
            ...
            #loss_accumulator += 0 * dino_global_crops_loss
        ...
        if do_ibot:
            ...
            #loss_accumulator += 0 * ibot_patch_loss


        if self.do_ctxor:
            with torch.no_grad():
                context_emb = torch.cat([
                    self.context_oracle(i.cuda())
                    for i in torch.tensor_split(images["context"], 128)
                ])

            ctxor_loss = torch.square(torch.stack(pctxor_local + pctxor_global) - context_emb).mean()

            loss_dict["ctxor_loss"] = ctxor_loss
            loss_accumulator += self.ctxor_loss_weight * ctxor_loss
    ...
    def fsdp_synchronize_streams(self):
        if self.need_to_synchronize_fsdp_streams:
            ...
            for attr in {"_unshard_stream", "_post_backward_stream", "_pre_unshard_stream", "_all_reduce_stream", "_default_stream"}:
                ...
                setattr(self.student.ctxor_head, attr, stream)

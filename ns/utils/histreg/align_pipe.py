...
from ns.utils.histreg.models.superglue import (
    match_superglue,
    basic_preprocessing,
    default_match_cf,
    np_to_tensor,
...
from ns.utils.histreg.models.bf import match_bf
...
def get_thumbnail(slide, target_downsample=64):
    ...
    thumb = slide.read_region(
        location=(0, 0), level=tn_level, size=slide.level_dimensions[tn_level]
    )
...
def get_sections_annot(he_annot):
    ...
    contours, _ = cv2.findContours(
        annot_mask.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
    )
...
def interpolate_crop_mask(he_proc_shape, he_mask):
    ...
    if desired_mask_size < ann_size:
        ...
        mask_resized = (
            F.interpolate(
                he_mask.to(torch.uint8).unsqueeze(0).unsqueeze(0),
                size=new_size,
                mode="nearest",
            )
            .squeeze(0)
...
def rescale_and_mask(he_proc, he_mask, he_mask_neg):
    ...
    if len(he_proc.shape) == 3:
        if he_proc.shape[-1] == 3:  # HWC
            ...
            bg_avg = he_proc[bg_coords2[:, 0], bg_coords2[:, 1], :].mean(axis=0)
...
def align_block(mask_annot, mask_annot_meta, match_cf, svs_root, out_dir):
    ...
    list_indices = [
        list(range(start, start + length)) for start, length in zip(starts, lengths)
    ]

    mask_annot_meta["sections_mask"] = list_indices
    curr_block = mask_annot_meta.explode("sections_mask").reset_index(drop=True)
    curr_block = curr_block[~(curr_block["comment"] == "RM")].reset_index(drop=True)
    ...
    curr_block_str = ".".join((curr_block["UM Accession"][0], curr_block["Block"][0]))
    ...
    align_viz_binary = [
        [None for _ in range(len(curr_block))] for _ in range(len(curr_he))
    ]
    align_viz_im = [[None for _ in range(len(curr_block))] for _ in range(len(curr_he))]
    ...
    for he_i, he_s in curr_he.iterrows():
        ...
        for ihc_i, ihc_s in curr_block.iterrows():
            ...
            if he_i == 0:
                all_binary.append(np.array(ihc_proc).squeeze().astype(np.float64))
                all_im.append(np.array(ihc_thumbnail).squeeze().astype(np.float64))
                all_scale.append(ihc_scale_j)

            if match_cf.get("bf_only", False):
                M, nm = match_bf(he_proc.to(torch.float), ihc_proc.to(torch.float))
                logging.info(
                    f"{he_s['Stain']} {he_i} -> {ihc_s['Stain']} {ihc_i}: {nm} BF dice"
                )
            else:
                M, nm = match_superglue(
                    he_proc.to(torch.float),
                ...
                logging.info(
                    f"{he_s['Stain']} {he_i} -> {ihc_s['Stain']} {ihc_i}: {nm} matches"
                )
            ...
            stacked_im = np.dstack(
                (transformed1, np.array(ihc_proc_orig).squeeze().astype(np.float64), b)
            )
            ...
            overlay = cv2.addWeighted(
                transformed2,
                0.2,
                np.array(ihc_thumbnail_orig).squeeze().astype(np.float64),
                0.8,
                0,
            ).astype(np.uint8)
    ...
    with open(opj(out_dir, f"{curr_block_str}_align.pkl"), "wb") as fd:
        pickle.dump(
            {"matrices": matrices, "num_matches": num_matches, "all_scale": all_scale},
            fd,
        )
    ...
    for im_i, (i, r) in zip(all_binary[: len(curr_block)], curr_block.iterrows()):
        im = (einops.repeat(im_i, "h w -> h w c", c=3) * 255).astype(np.uint8)
        im[:, :, [0, 2]] = 0
        axs[0][i + 1].imshow(im)
        axs[0][i + 1].axis("off")
...
def main():
    ...
    end_id = min((taskid + 1) * cf["infra"]["num_block_per_job"], len(blocks_df))
    ...
    for b in tqdm(blocks):
        ...
        try:
            ...
            mask_annot_meta = pd.read_csv(
                opj(cf["infra"]["annot_root"], "meta", f"{b}_sections_annot_meta.csv")
            )
            align_block(
                mask_annot,
                mask_annot_meta,

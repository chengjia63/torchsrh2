...
def collate_tile_data(samples_list, mask_ratio_tuple, mask_probability, n_tokens=None, mask_generator=None):
    ...
    collated_global_crops = einops.rearrange(collated_global_crops, "b v t c h w -> (b v t) c h w")
    
    collated_local_crops = torch.stack([torch.stack([s[0]["local_crops"][i] for i in range(n_local_crops)]) for s in samples_list])
    collated_local_crops = einops.rearrange(collated_local_crops, "b v t c h w -> (b v t) c h w")

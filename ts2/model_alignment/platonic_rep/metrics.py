...
class AlignmentMetrics:
    ...
    @staticmethod
    def masked_mutual_knn(feats_A, feats_B, topk, same_slide_mask):
        """
        Computes the mutual KNN accuracy.
        ...
        Returns:
            A float representing the mutual KNN accuracy
        """
        knn_A = compute_masked_nearest_neighbors(feats_A, same_slide_mask, topk)
        knn_B = compute_masked_nearest_neighbors(feats_B, same_slide_mask, topk)   
...
def compute_masked_nearest_neighbors(feats, mask, topk=1):
    """
    Compute the nearest neighbors of feats
    Args:
        feats: a torch tensor of shape N x D
    ...
    sim[mask] = -1e8
    knn = (
        sim.fill_diagonal_(-1e8).argsort(dim=1, descending=True)[:, :topk]
    )
    return knn

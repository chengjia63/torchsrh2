...
class MBMaskCollator(object):
    def __init__(self,
        ...
        self.height, self.width = input_size[0] // patch_size, input_size[
            1] // patch_size
        ...
        self._itr_counter = Value(
            'i', -1)  # collator is shared across worker processes
    ...
    def _sample_block_mask(self, b_size, acceptable_regions=None):
        ...
        mask = mask.squeeze()
        # --
        mask_complement = torch.ones((self.height, self.width),
                                     dtype=torch.int32)
    ...
    def __call__(self, batch):
        ...
        g = torch.Generator()
        g.manual_seed(seed)
        p_size = self._sample_block_size(generator=g,
                                         scale=self.pred_mask_scale,
                                         aspect_ratio_scale=self.aspect_ratio)
        e_size = self._sample_block_size(generator=g,
        ...
        collated_masks_pred = [[cm[:min_keep_pred] for cm in cm_list]
                               for cm_list in collated_masks_pred]
        collated_masks_pred = torch.utils.data.default_collate(
            collated_masks_pred)
        # --
        collated_masks_enc = [[cm[:min_keep_enc] for cm in cm_list]

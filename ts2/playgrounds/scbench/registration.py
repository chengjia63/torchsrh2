...
def fft_register(image1, image2):
	...
	"""
    registered_image = np.zeros((image1.shape[0], image1.shape[1], 2),
                                dtype=float)
    image3, scale, angle, _ = similarity(image1, image2)
    registered_image[:, :, 0] = image1
    registered_image[:, :, 1] = image3
    return registered_image


def register_mosaic(CH2_mosaic, CH3_mosaic, step=1000):
    """
	Registers the entire mosiac using patches of image size step.
	"""
    height = CH2_mosaic.shape[0]
    width = CH2_mosaic.shape[1]

    registered_image = np.zeros((CH2_mosaic.shape[0], CH2_mosaic.shape[1], 2),
                                dtype=float)
    for x in np.arange(0, width, step=step):
        for y in np.arange(0, height, step=step):
            ...
            registered_image[y:y + step, x:x + step, :] = fft_register(
                CH2_strip, CH3_strip)
    return registered_image
...
def compare_pre_post_reg(reference_image, pre_image, post_image):
    ...
    def subtract_channels(CH2, CH3):
        CH3_minus_CH2 = CH3.astype(float) - CH2.astype(float)
        CH3_minus_CH2[CH3_minus_CH2 < 0] = 0
        return CH3_minus_CH2.astype(np.uint8)

    # pre-registered image
    pre_reg_image = np.zeros(
        (reference_image.shape[0], reference_image.shape[1], 3),
        dtype=np.uint8)

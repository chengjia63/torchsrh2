...
from ts2.train.infra import (parse_args, read_process_cf, setup_infra_testing,
                             get_rank)
...
class SingleCellTempInferenceDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        for i, r in tqdm(slide_instances_df.iterrows()):
            ...
            pt_meta_fname = opj(data_root, r["institution"], r["patient"],
                                f"{r['patient']}_meta.json")
            ...
            with open(pt_meta_fname) as fd:
                slide_meta_all_inf = json.load(fd)["slides"][
                    r["mosaic"]]["predictions"]
            ...
            all_patches = pd.DataFrame(all_patches,
                                       columns=["patch_flat", "mmap_idx", "patch_type"])
            ...
            cp_filt = cp[cp["celltype"].isin({"nuclei", "mp"})
                         & (cp["score"] > 0.5) &
            ...
            cp_filt = pd.DataFrame({
                ...
                cp_filt["patch"],
                "proposal":
                zip(cp_filt["centroid_r"], cp_filt["centroid_c"])
            })
    ...
    def __getitem__(self, idx: int):
        ...
        rc_idx = (inst["proposal"][0] - self.tile_size_ // 2,
                  inst["proposal"][1] - self.tile_size_ // 2)

        mmap_path = opj(self.data_root_, inst["institution"], inst["patient"],
                        inst["mosaic"], "patches",
                        f"{inst['patient']}-{inst['mosaic']}-patches.dat")

        im = self.process_read_im_(mmap_path, inst["tensor_shape"],
                                   [inst["mmap_idx"]], rc_idx, self.tile_size_)
...
def main():
    ...
    dsets = {"SingleCellListInferenceDataset": SingleCellListInferenceDataset,
    ...
    inf_trainer = pl.Trainer(accelerator=device,
                             devices=1,

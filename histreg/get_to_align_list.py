...
def main(): 
    ...
    finished_annot_blocks = [f.removesuffix(".tiff") for f in finished_annot_blocks if f.endswith(".tiff")]
    previously_reg_blocks = os.listdir(block_align_prev)
    previously_reg_blocks = [f.removesuffix("_align.pkl") for f in previously_reg_blocks if f.endswith("_align.pkl")]

    to_reg = sorted(set(finished_annot_blocks).difference(previously_reg_blocks))

    print(f"num slide to reg: {len(to_reg)}")                                
    out_fname = opj(out_data_dir, f"to_reg_{datetime.now().strftime('%y%m%d')}.csv"             )
    print(out_fname)                                                       
    assert not os.path.exists(out_fname)                                   
                                                                                
    pd.DataFrame(to_reg, columns=["block"]).to_csv(out_fname,        
                                                         index=False)     
